  ...
  <span>
    <template v-if="Array.isArray(thumb)">
      <ul :class="listUlClass">
        <li v-for="(img, index) in thumb" :key="img.key" style="position:relative">
      ...
      <div v-show="fullViewerVisible" class="viewer-container viewer-backdrop viewer-fixed viewer-fade viewer-transition viewer-in" touch-action="none" style="z-index: 2015;">
        <div class="viewer-canvas" data-viewer-action="hide">
          <img ref="fullImage" @mousewheel="toggleZoomFullImage" 
            :src="Array.isArray(full) ? full[listActiveIndex].src : full" 
          ...
          <div class="viewer-navbar" v-show="multiple">
            <ul class="viewer-list viewer-transition" :style="listStyle">
              <li v-for="(item, index) in thumb"
                ...
                <img :src="item.src" role="button" data-viewer-action="view" data-index="0" data-original-url="images/tibet-1.jpg" alt="Cuo Na Lake" data-filled="true" style="width: 50px; height: 50px; transform: translateX(-10px);" />
              </li>
            </ul>
          </div>
        </div>
